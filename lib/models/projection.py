...
class Projector():
    ...
    def normalize(self, pixel_locations, h, w):
        ...
        normalized_pixel_locations = torch.clamp(normalized_pixel_locations, min=-1.1, max=1.1)  # [views, n_points, 2]

        return normalized_pixel_locations

    def compute_projections(self, xyz, train_cameras, resize_transform):
        '''
        ...
        '''
        original_shape = xyz.shape[:2] # n_ray, n_sample
        xyz = xyz.reshape(-1, 3)
        num_views = len(train_cameras)

        pixel_locations_view = []
        for i in range(num_views):
            xy = project_pose(xyz, train_cameras[i])# [n_ray*n_sample, 2]
            xy = torch.clamp(xy, -1.0, max(self.ori_image_size[0], self.ori_image_size[1]))
            trans = torch.as_tensor(resize_transform, dtype=torch.float, device=xyz.device)
            xy = do_transform(xy, trans)
            # xy = xy * torch.tensor(
            #     [w, h], dtype=torch.float, device=device) / torch.tensor(
            #     self.image_size, dtype=torch.float, device=device)
            # inbound = self.inbound(xy, h, w)# [n_ray*n_sample, 1]
            # inbounds.append(inbound)
            pixel_locations_view.append(xy)

        pixel_locations_points = torch.stack(pixel_locations_view, dim=0)#[views, n_points, 2]
        pixel_locations = torch.clamp(pixel_locations_points, min=-1e6, max=1e6)#[views, n_points, 2]

        # mask = projections[..., 2] > 0  # a point is invalid if behind the camera
        return pixel_locations.reshape((num_views,) + original_shape + (2,))
                # , inbound_out.reshape((num_views, ) + original_shape))
        ...
        # train_intrinsics = train_cameras[:, :9].reshape(-1, 3, 3)  # [n_views, 3, 3]
    ...
    def compute(self, xyz, train_imgs, train_cameras, featmaps=None, grid_sample=True, resize_transform=None):
        '''
        :param xyz: [n_rays, n_samples, 3]
        :param train_imgs: [1, n_views, h, w, 3]
        :param train_cameras: [{},{},...]
        :param featmaps: [n_views, d, h, w] ***********None
        ...
        # train_cameras = train_cameras.squeeze(0)  # [n_views, 25]
        ...
        pixel_locations = self.compute_projections(xyz, train_cameras, resize_transform)
        ...
        if featmaps is not None:
            ...
            rgb_feat_sampled = rgb_sampled
        # mask
        inbound = self.inbound(pixel_locations, h, w)
        mask = inbound.float().permute(1, 2, 0)[..., None]  # [n_rays, n_samples, n_views, 1]
