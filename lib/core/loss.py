...
class NeRFLoss(nn.Module):
    ...
    def forward(self, rgbs, gts, masks=[]):
        loss_nerf = 0
        if not masks:
            for rgb, gt in zip(rgbs, gts):
                loss_nerf += torch.mean((rgb - gt) ** 2)
        else:
            for rgb, gt, mask in zip(rgbs, gts, masks):
                loss_nerf += torch.sum(
                    mask.unsqueeze(-1) * (rgb - gt) ** 2) / (mask.sum() + 1e-6)
        return loss_nerf